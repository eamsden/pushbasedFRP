...
import           Control.Concurrent.MVar              -- MVars
import           Control.Concurrent                   -- Concurrency
                   (forkIO, threadDelay)
...
main = do (argS:_) <- getArgs
          ...
          reactiveVar <- newMVar $ initSFEval (eventHandler id) (sampleEvt) time learningSwitch
          forkIO $ let timerLoop = do rState <- takeMVar reactiveVar
                                      time <- getMonoTimeAsDouble
                                      ((), rState') <- runSFEvalT (step time) rState
                                      putMVar reactiveVar rState'
                                      threadDelay sampleTime
                                      timerLoop
                   in timerLoop
          let reactLoop = do mIn <- tryTakeMVar inputVar
                             case mIn of
                               Nothing -> return ()
                               Just (handle, tid, msg) -> do rState <- takeMVar reactiveVar
                                                             ((), rState') <- runSFEvalT (push $ svOcc (handle, tid, msg)) rState
                                                             putMVar reactiveVar rState'
                             reactLoop
          reactLoop
...
learningSwitch :: SVEvent (SwitchHandle EthernetFrame, TransactionID, SCMessage EthernetFrame) :~> SVEvent (IO ())
