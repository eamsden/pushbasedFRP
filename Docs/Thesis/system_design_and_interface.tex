...
In TimeFlies, the evaluation monad is a monad transformer. This permits it to be
used in conjunction with the {\tt IO} monad (or any other monad) to describe how
...
signal function. The monad is a state monad, and its state is a record with
fields for a mapping from outputs to handling actions, the current state of the
signal function, the last time the signal function was sampled, and the
currently-unsampled input. There are thus four type parameters to the monad's
type constructor: the input signal vector (to type the signal function and
unsampled input), the output signal vector (to type the signal function and
output handlers), the type of the underlying monad, and the monadic type
parameter. The type is shown in Figure~\ref{figure:evaluation_monad_types}.
